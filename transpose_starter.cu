...
transpose_parallel_per_element_tiled(float in[], float out[])
{
	...
	out[blockIdx.y * K + threadIdx.y + (blockIdx.x * K + threadIdx.x)*N] = in[blockIdx.x * K + threadIdx.x + (blockIdx.y * K + threadIdx.y)*N];
}
...
transpose_parallel_per_element_tiled_shared(float in[], float out[])
{
	...
	tile[y][x] = in[(blockIdx.x * K + x) + (blockIdx.y * K + y)*N];
	__syncthreads();
	out[(blockIdx.y * K + x) + (blockIdx.x * K + y)*N] = tile[x][y];
}
